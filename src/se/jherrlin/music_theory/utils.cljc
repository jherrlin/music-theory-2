...
;; Get the x, y positions of a set of tones
(let [matrix [[{:x 0, :tone #{:e}, :y 0, :out "3"}
               {:x 1, :tone #{:f}, :y 0}
               {:x 2, :tone #{:gb :f#}, :y 0}
              ...
              [{:x 0, :tone #{:a}, :y 1}
               {:x 1, :tone #{:bb :a#}, :y 1}
               {:x 2, :tone #{:b}, :y 1}
               {:x 3, :tone #{:c}, :y 1, :out "1", :root? true}
               {:x 11, :tone #{:g# :ab}, :y 1}
               {:x 12, :tone #{:a}, :y 1}
               {:x 13, :tone #{:bb :a#}, :y 1}
               {:x 14, :tone #{:b}, :y 1}
               {:x 15, :tone #{:c}, :y 1, :out "1", :root? true}]]
      s #{:c :g :e}]
  (->> matrix
       (apply concat)
       (filter (fn [{:keys [x y tone]}]
                 (seq (set/intersection tone s))))
       (map (fn [{:keys [x y tone] :as m}]
              (hash-map  {:x x :y y} m)))
       (into {})))
